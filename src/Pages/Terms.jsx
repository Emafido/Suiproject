      ...
      <div className="max-w-7xl mx-auto px-5 md:px-16 pt-40 grid grid-cols-1 lg:grid-cols-12 gap-20">
        ...
        <aside className="hidden lg:col-span-3 lg:block">
            <div className="sticky top-40">
                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-400 mb-10">Table of Contents</p>
                <nav className="flex flex-col gap-6 border-l border-gray-100 pl-6">
                    {Object.values(clauses).map((clause) => (
                        <button 
                            ...
                            className={`text-left text-sm font-bold transition-all duration-300 ${activeClause === clause.id ? 'text-gray-900 translate-x-2' : 'text-gray-400 hover:text-gray-600'}`}
                        >
                            {clause.title}
                        </button>
                    ))}
        ...
        <main className="lg:col-span-8">
            ...
            <p className="text-xl text-gray-500 font-medium leading-relaxed mb-20 max-w-2xl">
                ...
                Please read these terms carefully before interacting with the smart contracts.
            </p>
                ...
                {Object.values(clauses).map((clause, index) => (
                    <section key={clause.id} className="group">
                        <div className="flex items-baseline gap-4 mb-6">
                            <span className="font-heading font-black text-4xl text-gray-200 group-hover:text-green-500 transition-colors duration-500">0{index + 1}</span>
                            <h2 className="font-heading font-black text-3xl md:text-4xl text-gray-900 tracking-tightest">{clause.title}</h2>
                        </div>
            ...
            <div className="mt-32 p-12 bg-gray-50 rounded-[3rem] border border-gray-100">
                ...
                <p className="text-gray-500 mb-8 leading-relaxed">By connecting your wallet, you digitally sign these terms.</p>
                <Link to="/dashboard" className="inline-block bg-gray-900 text-white px-10 py-4 rounded-2xl font-black text-xs tracking-widest hover:bg-black transition">I AGREE & CONNECT</Link>
            </div>
        </main>
      </div>
    </div>
