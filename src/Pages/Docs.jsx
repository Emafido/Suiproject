      ...
      content: (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <p className="text-lg md:text-xl text-gray-500 font-medium leading-relaxed font-body">
            SuiSecureDrop is a decentralized, non-custodial protocol designed
            for secure asset transfers on the Sui network. It introduces a
          ...
          <div className="bg-gray-900 rounded-4xl md:rounded-[3rem] p-8 md:p-12 text-white shadow-2xl relative overflow-hidden group">
            ...
            <p className="font-body text-base md:text-lg leading-relaxed relative z-10">
              Every drop is a{" "}
              <span className="font-mono text-green-300 bg-white/5 px-2 py-0.5 rounded italic">
                Sui Object
              </span>
              . By moving assets into a contract-controlled object instead of
              directly to a recipient's address, we enable conditional logic
              that traditional transfers lack.
            </p>
          </div>
        </div>
      ),
      ...
      content: (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 font-body">
          <p className="text-lg md:text-xl text-gray-500 font-medium leading-relaxed">
            To interact with the protocol programmatically, you must call the{" "}
            <span className="font-mono bg-gray-100 px-2 rounded font-bold text-gray-900 text-sm md:text-base">
              create_drop
            </span>{" "}
            function.
          </p>
...
public entry fun create_drop<T: key + store>(
    ...
    ctx: &mut TxContext
          ...
          <p className="text-gray-500 leading-relaxed italic border-l-4 border-green-600 pl-6">
            The <span className="font-bold text-gray-900">pin_hash</span> is a
            SHA2-256 representation. The raw PIN never touches the ledger.
          </p>
        </div>
      ),
    },
    security: {
      ...
      content: (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <p className="text-lg md:text-xl text-gray-500 font-medium leading-relaxed font-body">
            Safety is the protocol's primary directive. The recall mechanism
            ensures that human error does not result in permanent loss of
            capital.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="p-8 border border-green-100 bg-green-50/30 rounded-4xl">
              ...
              <p className="text-sm text-gray-500 font-body leading-relaxed">
                Recalling a drop triggers an atomic transaction that destroys
            ...
            <div className="p-8 border border-blue-100 bg-blue-50/30 rounded-4xl">
              ...
              <p className="text-sm text-gray-500 font-body leading-relaxed">
                Only the original sender or the valid PIN holder can move
                assets. Admin keys do not exist.
              </p>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-white pb-20 font-body text-gray-900 antialiased overflow-x-hidden">
      ...
      >
        ...
        />
        <div
          className={`absolute right-0 top-0 h-full w-[80%] max-w-sm bg-white p-8 flex flex-col transition-transform duration-500 ${
            isMenuOpen ? "translate-x-0" : "translate-x-full"
          }`}
        >
          <button
            onClick={() => setIsMenuOpen(false)}
            className="self-end w-12 h-12 bg-gray-50 rounded-full mb-10"
          >
            <i className="fa-solid fa-xmark"></i>
          </button>
          <p className="text-[10px] font-black uppercase tracking-widest-xl text-gray-400 mb-8 font-body">
            Navigate Docs
          </p>
          <nav className="flex flex-col gap-8">
            {Object.keys(sections).map((key) => (
              <button
                ...
                className={`text-left font-heading font-black text-4xl tracking-tightest transition ${
                  activeSection === key ? "text-green-600" : "text-gray-300"
                }`}
              >
                {sections[key].title}
              </button>
        ...
        <aside className="hidden lg:col-span-3 lg:block">
          <div className="sticky top-40">
              ...
              {Object.keys(sections).map((key) => (
                <button
                  ...
                  className={`text-left font-heading font-black text-3xl tracking-tightest transition-all duration-500 ${
                    activeSection === key
                      ? "text-green-600 translate-x-3 scale-105"
            ...
            <div className="mt-20 p-8 bg-gray-50 rounded-[2.5rem] border border-gray-100 group hover:border-green-200 transition">
              <p className="text-xs font-black text-gray-900 mb-2 uppercase tracking-widest font-body">
                Dev Support
              </p>
              <p className="text-xs text-gray-500 leading-relaxed font-body font-medium">
                Have technical questions about Sui Move? Join our engineering
                hub.
              </p>
              <a
                href="#"
                className="inline-block mt-4 text-[10px] font-black text-green-600 uppercase tracking-[0.2em] group-hover:translate-x-1 transition-transform"
              >
                Discord Community →
              </a>
            </div>
        ...
        <div className="lg:hidden flex overflow-x-auto gap-8 pb-4 border-b border-gray-100 no-scrollbar sticky top-24 bg-white/80 backdrop-blur-md pt-4 -mx-5 px-5 z-40">
          {Object.keys(sections).map((key) => (
            <button
              key={key}
              onClick={() => setActiveSection(key)}
              className={`whitespace-nowrap font-heading font-black text-xl tracking-tightest transition ${
                activeSection === key ? "text-green-600" : "text-gray-300"
              }`}
            >
              {sections[key].title}
          ...
          <div className="max-w-4xl">
            ...
            <div className="mt-20 md:mt-32 pt-12 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-6">
              <div className="flex flex-col items-center sm:items-start">
                ...
                <p className="text-[10px] font-black text-gray-900 uppercase tracking-[0.3em] font-body mt-1">
                  Built for Sui Mainnet v1.0.0
                </p>
              </div>
              <div className="flex gap-4">
                <button className="w-14 h-14 rounded-2xl border border-gray-100 flex items-center justify-center text-gray-300 hover:bg-gray-50 transition active:scale-90">
