...
const Dashboard = () => {
  ...
  const { balance, items, address, refetch: refetchInventory } = useMyInventory();
    ...
    <div className="min-h-screen bg-gray-50 pb-16 font-body text-gray-900 antialiased overflow-x-hidden">
      ...
      */}
      <nav className="flex items-center px-6 md:px-12 py-5 bg-white border-b border-gray-100 shadow-sm sticky top-0 z-[60] justify-between">
        ...
        <div className="hidden lg:flex gap-10 items-center text-xs font-black uppercase tracking-[0.2em] text-gray-400">
            ...
            <Link to="/claim" className="hover:text-green-600 transition">Inbox</Link>
            <Link to="/history" className="hover:text-green-600 transition">History</Link>
      ...
      */}
      <div className={`fixed inset-0 z-[100] transition-all duration-500 lg:hidden ${isMenuOpen ? 'visible' : 'invisible'}`}>
        ...
        <div 
          className={`absolute inset-0 bg-gray-900/60 backdrop-blur-md transition-opacity duration-500 ${isMenuOpen ? 'opacity-100' : 'opacity-0'}`} 
          onClick={() => setIsMenuOpen(false)} 
        />
        
        {/* Side Menu Drawer */}
        <div className={`absolute right-0 top-0 h-full w-[85%] max-w-sm bg-white shadow-2xl transition-transform duration-500 ease-in-out p-8 flex flex-col ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
            <div className="flex justify-between items-center mb-12">
                <p className="font-heading font-black text-2xl tracking-tightest text-gray-900">Menu</p>
                <button onClick={() => setIsMenuOpen(false)} className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center text-gray-900">
                    <i className="fa-solid fa-xmark"></i>
                </button>
            </div>
            
            <nav className="flex flex-col gap-8 flex-1">
                <Link to="/dashboard" onClick={() => setIsMenuOpen(false)} className="text-4xl font-heading font-black text-green-600 tracking-tightest">Dashboard</Link>
                <Link to="/claim" onClick={() => setIsMenuOpen(false)} className="text-4xl font-heading font-black text-gray-900 tracking-tightest">Inbox</Link>
                <Link to="/history" onClick={() => setIsMenuOpen(false)} className="text-4xl font-heading font-black text-gray-900 tracking-tightest">History</Link>
            </nav>
        ...
        {!isConnected ? (
          <div className="flex flex-col items-center justify-center text-center py-16 md:py-24 bg-white rounded-[3rem] border border-gray-100 shadow-2xl shadow-gray-200/40 px-6 animate-in fade-in duration-700">
              ...
              <p className="text-gray-500 max-w-md mb-12 text-lg md:text-xl font-medium leading-relaxed">
                  Connect your wallet to start sending PIN-protected care packages and manage your vault.
              </p>
              ...
              <div className="w-full max-w-md bg-gray-50 rounded-[2rem] p-8 text-left border border-gray-100 mb-12">
                  <p className="text-[11px] font-black text-gray-400 uppercase tracking-widest mb-6 flex items-center gap-2">
                      <i className="fa-solid fa-mobile-screen"></i> Mobile Instructions
                  </p>
                  <ul className="space-y-5 text-sm text-gray-600 font-bold">
                      <li className="flex gap-4 items-start">
                          <span className="bg-white text-gray-900 rounded-xl w-8 h-8 flex items-center justify-center shadow-sm shrink-0">1</span>
                          <span className="leading-snug">Open <strong>Sui Wallet App</strong> on your phone.</span>
                      </li>
                      <li className="flex gap-4 items-start">
                          <span className="bg-white text-gray-900 rounded-xl w-8 h-8 flex items-center justify-center shadow-sm shrink-0">2</span>
                          <span className="leading-snug">Tap the <strong>Browser (Globe Icon)</strong> at the bottom.</span>
                      </li>
                      <li className="flex gap-4 items-start">
                          <span className="bg-white text-gray-900 rounded-xl w-8 h-8 flex items-center justify-center shadow-sm shrink-0">3</span>
                          <span className="leading-snug">Paste this URL and click <strong>Connect</strong>.</span>
                      </li>
                  </ul>
              </div>
            ...
            <div className="lg:col-span-5 space-y-10">
              <div>
                <p className="mb-4 text-xs font-black uppercase tracking-widest text-gray-400 flex items-center gap-2">
                    <i className="fa-solid fa-wallet text-gray-400"></i> Portfolio Status
                </p>
                <div className="relative overflow-hidden border border-gray-700 rounded-[2.5rem] shadow-2xl p-10 md:p-12 bg-gradient-to-br from-gray-800 to-gray-900 text-white group">
                  ...
                  <p className="text-gray-400 text-[10px] font-black uppercase tracking-[0.3em]">Current Assets</p>
                  <h2 className="font-heading text-5xl md:text-7xl font-black mt-2 mb-10 tracking-tightest leading-none">
                    {balance} <span className="text-xl md:text-2xl text-green-400 font-bold">SUI</span>
                  </h2>
                  <div className="flex items-center gap-3 bg-white/5 border border-white/10 w-fit px-5 py-2.5 rounded-2xl backdrop-blur-md">
                    ...
                    <p className="text-xs text-gray-300 font-mono font-bold tracking-tight">{formatAddress(address)}</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between items-center my-6">
                  <p className="text-xs font-black uppercase tracking-widest text-gray-400">Vault Artifacts</p>
                  <button onClick={mintDemoItem} className="text-[10px] bg-gray-900 text-white px-5 py-2 rounded-xl font-black tracking-widest shadow-xl active:scale-95 transition">+ MINT</button>
                </div>
                <div className="grid grid-cols-1 gap-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                  {items.length === 0 ? (
                    <div className="p-16 text-center border-2 border-dashed border-gray-100 rounded-[2.5rem] bg-white shadow-sm">
                       <p className="text-gray-400 text-sm font-bold italic">No items stored in vault.</p>
                    </div>
                  ) : (
                    items.map((item) => (
                        <div key={item.data.objectId} className="flex border border-gray-100 bg-white rounded-[2rem] shadow-sm p-6 items-center justify-between hover:border-green-300 transition-all group">
                            <div className="flex items-center gap-5 overflow-hidden">
                                <div className="w-14 h-14 bg-gray-50 rounded-2xl flex items-center justify-center text-2xl group-hover:rotate-12 transition-transform">⚔️</div>
                                <div className="overflow-hidden">
                                    <p className="font-heading font-black text-gray-800 text-lg truncate tracking-tight">{item.data.content?.fields?.name || "Artifact"}</p>
                                    <p className="text-[10px] font-mono font-bold text-gray-400 mt-0.5 tracking-tighter uppercase">{item.data.objectId.slice(0,25)}...</p>
                                </div>
                            </div>
                        </div>
                    ))
            ...
            <div className="lg:col-span-7 space-y-10">
              <div className="flex justify-between items-center">
                <p className="text-xs font-black uppercase tracking-widest text-gray-400 flex items-center gap-2">
                    <i className="fa-solid fa-paper-plane text-gray-400 text-lg"></i> Active Deliveries
                </p>
                <Link to="/create" className="flex items-center gap-3 bg-green-600 text-white px-8 py-3.5 rounded-[1.5rem] font-black text-xs shadow-2xl shadow-green-100 active:scale-95 transition transform">
              ...
              <div className="grid grid-cols-1 gap-6">
                {isPending ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-28 border-2 border-dashed border-gray-100 rounded-[3.5rem] bg-white text-center shadow-inner">
                        ...
                        <p className="text-gray-400 font-black text-lg mb-2">No active drops found.</p>
                        <Link to="/history" className="text-green-600 font-black text-xs hover:underline uppercase tracking-widest-xl">
                    ...
                    myDrops.map((drop) => {
                      ...
                      return (
                        <div key={drop.id} className="relative flex flex-col border border-gray-100 bg-white rounded-[3.5rem] p-10 shadow-sm hover:shadow-2xl transition-all duration-700 group">
                            <div className="flex flex-col xl:flex-row justify-between gap-10 mb-10">
                              <div className="flex gap-8 items-center">
                                  ...
                                  <div>
                                      <p className="font-heading font-black text-gray-900 text-4xl tracking-tightest leading-none">{(parseInt(drop.balance || drop.amount) / 1e9).toFixed(2)}</p>
                                      <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest-xl mt-2">SUI Locked</p>
                                  </div>
                              </div>
                              {itemCount > 0 && (
                            ...
                            <div className="bg-gray-50 rounded-[2.5rem] p-8 mb-8 border border-gray-100 space-y-4">
                              <div className="flex flex-col gap-2">
                                  <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Intended Recipient</span>
                              ...
                              <button 
                                  onClick={() => {
                                      if(window.confirm("Recall this package?")) {
                                          recallDrop({
                                              dropId: drop.id,
                                              onSuccess: () => { refetchDrops(); refetchInventory(); },
