...
const History = () => {
    ...
    events?.data
      ?.map((event) => {
        ...
        return {
          ...
          party: displayParty,
          amount: (
            parseInt(data.amount || data.balance || 0) / 1_000_000_000
          ).toFixed(2),
        };
      })
      .filter(Boolean) || [];
  ...
  const formatDate = (ms) =>
    new Date(parseInt(ms)).toLocaleString([], {
      dateStyle: "medium",
      timeStyle: "short",
    });
    ...
    <div className="min-h-screen bg-gray-50 pb-10">
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        <div className="pt-8 pb-6">
          <Link
            to="/dashboard"
            className="text-gray-400 font-bold hover:text-green-600 transition flex items-center gap-2 mb-4 text-sm"
          >
                ...
                >
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-gray-400 font-mono mt-0.5 truncate max-w-45 sm:max-w-none">
                        {item.party
                          ...
                          : ""}
                        {item.party
                          ? `${item.party.slice(0, 6)}...${item.party.slice(
                              -4
                            )}`
                          : "Care Package"}
                      </p>
                      <p className="text-[10px] text-gray-300 mt-1 uppercase font-bold tracking-widest">
                  ...
                  <div className="flex flex-row sm:flex-col justify-between items-center sm:items-end border-t sm:border-t-0 pt-3 sm:pt-0">
                    ...
                    >
                      {item.isPositive ? "+" : "-"}
                      {item.amount}{" "}
                      <span className="text-sm font-bold">SUI</span>
                    </p>
                    <a
