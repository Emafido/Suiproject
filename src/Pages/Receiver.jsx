...
const Receiver = () => {
  ...
  const handleClaim = (dropId) => {
    ...
    claimDrop({
      ...
      onSuccess: () => {
        // 1. Victory Animation
        confetti({ particleCount: 150, spread: 70, colors: ['#10B981', '#3B82F6'] });
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-3xl mx-auto">
        <div className="flex justify-between items-center mb-8">
            <Link to="/dashboard" className="text-gray-500 font-bold hover:text-green-600 transition flex items-center gap-2">
        ...
        <div className="space-y-4">
          ...
          {!isPending && incomingDrops.length === 0 && (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-12 text-center animate-in fade-in">
                ...
                <h3 className="text-lg font-bold text-gray-800">No pending drops</h3>
                <p className="text-gray-500 text-sm mt-2">
                    Any packages sent to <span className="font-mono bg-gray-100 px-1 rounded">{account?.address?.slice(0,6)}...</span> will appear here.
                </p>
            </div>
          )}
          ...
          {incomingDrops.map(drop => (
             <div key={drop.id} className="bg-white p-6 rounded-2xl shadow-md border border-gray-100 flex flex-col md:flex-row justify-between items-center gap-6 hover:border-green-400 transition animate-in slide-in-from-bottom-2">
               ...
               <div className="flex gap-4 items-center w-full md:w-auto">
                  ...
                  <div>
                    <p className="font-bold text-gray-800 text-lg">Secure Package</p>
                    <p className="text-green-600 font-black text-xl">{(parseInt(drop.balance)/1e9).toFixed(2)} SUI</p>
                    {drop.items.length > 0 && (
               ...
               <div className="flex gap-2 w-full md:w-auto bg-gray-50 p-2 rounded-xl">
                 <input 
                    ...
                    onChange={(e) => setPinInputs({...pinInputs, [drop.id]: e.target.value})} 
